...
def test_to_device_inverts_to_image():
    for point in (MapPoint(0.0, 0.0), MapPoint(-1975.0, 320.5), MapPoint(1234.0, -999.0)):
        ...
        device_x, device_y = SYSTEM.to_device(x, y)
        assert device_x == pytest.approx(point.x)
        assert device_y == pytest.approx(point.y)
...
def test_to_device_without_scaling_or_border():
    ...
    assert system.to_device(0.0, 0.0) == (0.0, 30.0)
    assert system.to_device(2.0, 3.0) == (20.0, 0.0)
